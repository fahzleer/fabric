...
export interface CfPaymentConfig {
...
export function loadConfig(): CfPaymentConfig {
  const missing = (REQUIRED_VARS as readonly string[]).filter((key) => !process.env[key]);
  if (missing.length > 0) {
    throw new Error(`[cf-payment/config] Missing required env vars: ${missing.join(", ")}`);
  }

  const gateway = (process.env.PAYMENT_GATEWAY ?? "mock") as PaymentGateway;

  if (gateway === "omise" && !process.env.OMISE_SECRET_KEY) {
    throw new Error("[cf-payment/config] OMISE_SECRET_KEY is required when PAYMENT_GATEWAY=omise");
  }
